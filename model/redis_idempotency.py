...
class RedisIdempotencyStore:
    def __init__(self, client: Any, *, namespace: str = "model-idempotency", ttl_seconds: int = 300):
    ...
    def get(self, *, tenant_id: str, key: str, fingerprint: str) -> object | None:
        try:
            ...
            raise IdempotencyUnavailableError("idempotency backend unavailable") from exc

    def set(self, *, tenant_id: str, key: str, fingerprint: str, result: object) -> object:
        try:
            self.client.setex(
                self._key(tenant_id, key), self.ttl_seconds,
    ...
    def get_or_compute(self, *, tenant_id: str, key: str, fingerprint: str, producer) -> object:
