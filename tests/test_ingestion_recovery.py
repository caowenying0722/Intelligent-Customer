...
def test_persisted_jobs_support_cancel_and_orphan_recovery() -> None:
    ...
    running = IngestionJob(
        job_id=uuid4(), tenant_id="tenant-a", idempotency_key="running",
        status=IngestionJobStatus.RUNNING, created_at=datetime.now(timezone.utc),
    )
    ...
    try:
        cancelled = repository.request_cancel(tenant_id="tenant-a", job_id=queued.job_id)
