...
def _service(root):
    ...
    return (
        DocumentIngestionService(SecureUploadStorage(root), jobs, registry),
        jobs,
        registry,
    )
...
def test_document_lifecycle_reaches_active_and_reuses_hash() -> None:
    ...
    try:
        first = service.submit_document(
            ...
            index_version="idx-1",
            operation=lambda path, upload, record: "ok",
        )
        assert (
            _finish(jobs, "tenant-a", first.job.job_id).status
            == IngestionJobStatus.COMPLETED
        )
        assert (
            registry.get(
                tenant_id="tenant-a", document_id=first.document.document_id
            ).status
        ...
        duplicate = service.submit_document(
            ...
            index_version="idx-2",
            operation=lambda path, upload, record: "bad",
        )
...
def test_document_lifecycle_persists_original_failure_message() -> None:
    ...
    try:
        submission = service.submit_document(
            ...
            embedding_dimension=3,
            index_version="idx-1",
            operation=lambda path, upload, record: (_ for _ in ()).throw(
                ValueError("boom")
            ),
        )
