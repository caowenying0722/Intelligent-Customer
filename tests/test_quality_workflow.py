...
def test_quality_workflow_validates_manifest_and_prepares_artifacts() -> None:
    ...
    assert "python -m pip_audit -r requirements-worker.lock" in workflow
    assert "docker build --file Dockerfile.worker" in workflow
    assert "python scripts/run_worker_ingestion_smoke.py" in workflow
    assert "INGESTION_WORKER_BACKEND=celery" in workflow
