...
Prometheus 接口只输出 HTTP、模型网关和入库 Worker 的聚合计数、固定延迟桶、缓存/用量聚合值和健康状态。Worker 指标包含队列深度、活动任务、队列等待/处理耗时、重试、完成、失败和取消；标签仅使用固定的 HTTP status class、延迟桶和已配置 provider 名称，provider series 最多 32 个。不会输出 tenant、job、用户、会话、请求路径、prompt、文档正文或凭据。
