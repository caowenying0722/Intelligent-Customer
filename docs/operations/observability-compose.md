...
镜像版本固定在 Compose 文件中，首次启动仍需要拉取外部镜像；Collector、Prometheus、Grafana 镜像已在本机独立 health smoke 中返回 200，但 API 镜像 build 在 BuildKit 导出阶段遇到 daemon EOF，随后 Docker Desktop restart 也未恢复 daemon，不能宣称完整 profile 已健康运行。执行 `docker compose --profile observability config --quiet` 可先验证静态配置。
