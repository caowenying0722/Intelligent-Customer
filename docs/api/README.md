...
- Chat 会从当前 tenant 的 conversation repository 读取最近最多 20 条、总计最多 8000 字符的历史；实现 `run_with_history`/`stream_with_history` 的 Agent 会收到结构化历史，ModelGateway 会把同样的有界上下文纳入请求/缓存键。SSE 的 `metadata` 事件返回 conversation ID，并在完成后写回 user/assistant 消息；旧的单消息 Agent 保持兼容。Streamlit 的 HTTP 模式会复用该 ID，进程内模式仍只保留 session state。
