...
| TD-025 | 尚无结构化日志、trace 或完整 metrics | Medium | 故障无法按请求关联，无法观测模型/RAG/工具耗时 | `utils/logger_handler.py:14-50`、`src/app/main.py`、`src/app/observability/metrics.py` | API middleware 已注入并回传 request ID；HTTP 与模型网关已有 Prometheus 聚合出口；后续 OTel 与 RAG/工具耗时指标，继续控制标签基数 | 2/8 | 部分完成 |
