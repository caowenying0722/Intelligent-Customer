...
| TD-009 | FastAPI SSE、持久化会话和完整错误映射尚未实现 | High | 当前聊天 API 仅支持非流式内存边界，尚不能提供完整 API-first 服务 | `src/app/main.py`、`src/app/application/chat.py`、`app.py` | 应用工厂、request ID、liveness/readiness、严格 chat schema、fake Agent 和 timeout 错误映射已完成；继续增加 SSE、取消和会话 repository | 2/3 | 部分完成 |
