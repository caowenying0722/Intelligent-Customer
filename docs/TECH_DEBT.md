...
| TD-025 | 尚无结构化日志、trace 或完整 metrics | Medium | 故障无法按请求关联，无法观测模型/RAG/工具耗时 | `utils/logger_handler.py:14-50`、`src/app/main.py`、`src/app/application/chat.py`、`rag/rag_service.py`、`agent/tools/agent_tools.py`、`src/app/application/ingestion.py`、`src/app/observability/metrics.py`、`src/app/observability/tracing.py` | API middleware 已注入 request ID、W3C traceparent、HTTP/Agent/LLM/RAG/工具/当前进程 Worker SDK span；HTTP 与模型网关已有 Prometheus 聚合出口，生产 token/OTLP HTTPS 保护；可选 OTLP exporter 已有 timeout 和 endpoint 校验；worker 关闭会先 drain 已提交任务再释放持久化 store；后续 Collector、重启任务 context 持久化和 worker metrics | 2/8 | 部分完成 |
