...
| TD-025 | 尚无结构化日志、trace 或完整 metrics | Medium | 故障无法按请求关联，无法观测模型/RAG/工具耗时 | `utils/logger_handler.py:14-50`、`src/app/main.py`、`src/app/observability/metrics.py`、`src/app/observability/tracing.py` | API middleware 已注入 request ID、W3C traceparent 和 `http.request` SDK span；HTTP 与模型网关已有 Prometheus 聚合出口，生产 token 保护；后续 OTLP exporter/Collector 与 Agent/RAG/工具 span，继续控制标签基数 | 2/8 | 部分完成 |
