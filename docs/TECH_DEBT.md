...
| TD-009 | FastAPI 持久化会话尚未实现 | High | 当前聊天 API 仍是内存边界，SSE 对同步 Agent 的取消只能在适配器层停止等待 | `src/app/main.py`、`src/app/application/chat.py`、`src/app/domain/conversations.py` | 应用工厂、request ID、健康检查、统一错误契约、严格 schema、fake Agent、timeout、SSE、会话查询、内存 repository 和原生异步取消边界已完成；继续增加 PostgreSQL repository | 2/3 | 部分完成 |
