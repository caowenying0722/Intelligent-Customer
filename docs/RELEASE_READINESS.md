...
| 检查 | 实际结果 | 说明 |
|---|---|---|
| `python -m pytest -q` | 通过：304 passed，25 subtests | 默认不调用付费模型 |
...
| W3C/HTTP/Agent span API smoke | 通过：5 个测试 | 严格校验/生成上下文并回传新 span；仅有界本地 exporter，未接入 OTLP |
...
- Compose 目前只有 API；PostgreSQL、Redis、Qdrant、Worker、OpenTelemetry Collector、Prometheus server、Grafana 和 trace backend 尚未纳入。API 已提供可抓取的 HTTP/模型网关 Prometheus 文本端点、W3C traceparent 传播和 `http.request`/`agent.run` SDK span，但尚无 RAG/工具耗时指标。
