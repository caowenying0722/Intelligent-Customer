...
2. `docker build --tag intelligent-customer-api:local .` 已重新执行：Python 3.10 基础镜像和依赖下载/安装完成，但约 889 秒后在 BuildKit 镜像导出阶段收到 Docker daemon EOF；最终没有生成可用镜像。随后 Docker Desktop 报无法启动，不能宣称 Docker 镜像可发布。
