...
| 检查 | 实际结果 | 说明 |
|---|---|---|
| `python -m pytest -q` | 通过：307 passed，25 subtests | 默认不调用付费模型 |
...
| W3C/HTTP/Agent/LLM/SSE/RAG span smoke | 通过：8 个测试 | 严格校验/生成上下文并回传嵌套 span；仅有界本地 exporter，未接入 OTLP |
...
- Compose 目前只有 API；PostgreSQL、Redis、Qdrant、Worker、OpenTelemetry Collector、Prometheus server、Grafana 和 trace backend 尚未纳入。API 已提供可抓取的 HTTP/模型网关 Prometheus 文本端点、W3C traceparent 传播和 HTTP/Agent/LLM/RAG SDK span，但尚无工具耗时指标。
