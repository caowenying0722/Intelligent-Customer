...
| 检查 | 实际结果 | 说明 |
|---|---|---|
| `python -m pytest -q` | 通过：294 passed，25 subtests | 默认不调用付费模型 |
| `coverage run -m pytest -q && coverage report` | 通过：总覆盖率 58%，门槛 41% | 当前本地基线 |
| `python -m ruff format --check .` | 通过 | 206 个 Python 文件已格式化 |
...
| `GET /metrics/prometheus` 集成测试 | 通过：3 个测试 | 有界模型网关聚合指标，无 tenant/user/request/prompt 内容 |
...
- Compose 目前只有 API；PostgreSQL、Redis、Qdrant、Worker、OpenTelemetry、Prometheus server、Grafana 和 trace backend 尚未纳入。API 已提供可抓取的 Prometheus 文本端点，但尚无 RAG/工具耗时指标。
