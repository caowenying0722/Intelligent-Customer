...
- 阶段 11 第三十个目标进行中：补齐 API 访问日志的结构化脱敏基线，只记录 method/status/duration/request_id/trace_id，禁止 Authorization、Cookie、query、prompt、正文和供应商原始错误进入日志，并增加 middleware 回归测试。
