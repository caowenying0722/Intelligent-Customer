...
| OpenTelemetry | 部分实现 | API 有 W3C `traceparent`、`http.request`、`agent.run`/`agent.stream` 和 Gateway `llm.generate` SDK span/有界本地 exporter；尚无 OTLP Collector、RAG/工具 span 或 backend |
