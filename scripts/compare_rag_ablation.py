...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run baseline vs reranked RAG ablation experiments."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="output/evaluation_ablation",
    ...
    parser.add_argument(
        "--no-generate", action="store_true", help="Skip LLM answer generation."
    )
    parser.add_argument(
        "--answer-mode",
...
def main() -> None:
    ...
    comparison_dir = compare_summaries(
        ...
        focus_metrics=[
            metric.strip() for metric in args.focus_metrics.split(",") if metric.strip()
        ],
    )
